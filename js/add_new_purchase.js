...
function addPurchase() {
  ...
  if(!notNull(suppliers_name.value, "supplier_name_error"))
    ...
    for(var i = 0; i < row_count - 1; i++) {
      ...
      addMedicineStock(medicineStockRow[i].name, medicineStockRow[i].batch_id, medicineStockRow[i].expiry_date, medicineStockRow[i].quantity, medicineStockRow[i].mrp, medicineStockRow[i].rate);
    }
...
function addMedicineStock(name, batch_id, expiry_date, quantity, mrp, rate) {
  ...
  xhttp.onreadystatechange = function() {
    if(xhttp.readyState = 4 && xhttp.status == 200)
      xhttp.responseText;
      // alert("ADDED MEDICINE STOCK " + xhttp.responseText);
  };
